...
void eof_prepare_note_menu(void)
{
	...
	if(eof_song && eof_song_loaded)
	{
		if(vselected)
		{
			...
			{
				eof_note_menu[6].flags = 0;
			}
			else
			{
				eof_note_menu[6].flags = D_DISABLED;
			}
			eof_note_menu[9].flags = 0;
			eof_note_menu[10].flags = 0;
		...
		if(inll)
		{
			eof_lyric_line_menu[1].flags = 0;
			eof_lyric_line_menu[4].flags = 0;
		...
		{
			eof_note_menu[0].flags = D_DISABLED;
...
static void eof_split_lyric(int lyric)
{
	...
	if(eof_song->vocal_track->lyric[lyric]->note >= 36 && eof_song->vocal_track->lyric[lyric]->note <= 84)
	{
	...
	if(eof_vocals_selected)
	{
		...
		for(i = 0; i < eof_song->vocal_track->lyrics; i++)
		{
			if(eof_selection.track == EOF_TRACK_VOCALS && eof_selection.multi[i])
			{
				...
				if(eof_song->vocal_track->lyric[i]->note <= 36 && dir > 0)
				{
					return 0;
				}
				else if(eof_song->vocal_track->lyric[i]->note >= 84 && dir < 0)
				{
					return 0;
				}
			}
