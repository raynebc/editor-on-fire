...
int eof_menu_song_properties(void)
{
	...
	if(eof_popup_dialog(eof_song_properties_dialog, 2) == 23)
	{	//User clicked OK
...
int eof_manage_rs_phrases_add_level(DIALOG * d)
{
	...
	return eof_manage_rs_phrases_add_or_remove_level(1);	//Call the add level logic
}

int eof_manage_rs_phrases_remove_level(DIALOG * d)
{
	if(!d)
	{	//Satisfy Splint by checking value of d
		return D_O_K;
	...
	return eof_manage_rs_phrases_add_or_remove_level(-1);	//Call the remove level logic
}

int eof_manage_rs_phrases_add_or_remove_level(int function)
{
...
int eof_menu_song_rs_popup_messages(void)
{
	...
	tracknum = eof_song->track[eof_selected_track]->tracknum;
	tp = eof_song->pro_guitar_track[tracknum];
	eof_pro_guitar_track_sort_popup_messages(tp);	//Re-sort the popup messages
...
int eof_rs_popup_messages_edit(DIALOG * d)
{
	...
	{	//Free previously allocated strings
		free(eof_rs_popup_messages_list_strings[i]);
	}
	free(eof_rs_popup_messages_list_strings);
	eof_rs_popup_messages_list_strings = NULL;
...
int eof_rs_popup_messages_delete_all(DIALOG * d)
{
	...
	{	//For each popup message (in reverse order)
		...
		eof_pro_guitar_track_delete_popup_message(eof_song->pro_guitar_track[tracknum], i - 1);
	}
