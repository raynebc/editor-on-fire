...
int eof_get_previous_note(int cnote)
{
	...
	for(i = cnote - 1; i >= 0; i--)
	{
		if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->type == eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->type)
		{
			return i;
		}
	}
...
int eof_note_is_hopo(int cnote)
{
	...
	{
		if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->flags & EOF_NOTE_FLAG_NO_HOPO)
		{
			return 0;
		}
		if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->flags & EOF_NOTE_FLAG_F_HOPO)
		{
			return 1;
		}
	}
	...
	{
		if((eof_song->beat[i]->pos <= eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->pos) && eof_song->beat[i + 1]->pos > eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->pos)
		{
			beat = i;
			break;
		}
	...
	if(cnote > 0)
	{
		...
		{
			delta = eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->pos - eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote]->pos;
			if((delta <= (hopo_delta * scale)) && (eof_note_count_colors(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote]) == 1) && (eof_note_count_colors(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]) == 1) && (eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote]->note != eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->note))
			{
				return 1;
			}
		}
		...
		{
			delta = eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->pos - (eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote]->pos + eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote]->length);
			if((delta <= hopo_delta * scale) && !(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote]->note & eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote]->note))
			{
				return 1;
			}
		}
//		allegro_message("bpm = %f\nscale = %f\ndelta = %f\npnote = %d(%d), cnote = %d(%d)", bpm, scale, delta, pnote, eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[pnote].pos, cnote, eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[cnote].pos);
	}
	return 0;
}
...
void eof_determine_hopos(void)
{
	...
	for(i = 0; i < eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->notes; i++)
	{
		...
		/* clear the flags */
		if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->flags & EOF_NOTE_FLAG_HOPO)
		{
			eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->flags ^= EOF_NOTE_FLAG_HOPO;
		}
		if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->flags & EOF_NOTE_FLAG_SP)
		{
			eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->flags ^= EOF_NOTE_FLAG_SP;
		}
//		eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->flags = (eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->flags & EOF_NOTE_FLAG_CRAZY);
		...
		{
			if((eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->pos >= eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->solo[j].start_pos) && (eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->pos <= eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->solo[j].end_pos))
			{
				so[j] = 1;
			}
		}
	...
	for(j = eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->star_power_paths - 1; j >= 0; j--)
	{
		if(!sp[j])
		{
	...
	for(j = eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->solos - 1; j >= 0; j--)
	{
		if(!so[j])
		{
...
int eof_figure_difficulty(void)
{
	...
	if(eof_vocals_selected)
	{
		...
		for(i = 0; i < eof_song->vocal_track[0]->lyrics; i++)
		{
			if(eof_song->vocal_track[0]->lyric[i]->note != 0)
			{
				break;
			}
		}
		...
		if((i < eof_song->vocal_track[0]->lyrics) && (eof_song->vocal_track[0]->lines > 0))
		{
			return 0;
		}
		return -1;
	...
	for(i = 0; i < eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->notes; i++)
	{
		nt[(int)eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->type] = 1;
	}
...
{
	return eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->notes;
}
	...
	for(i = 0; i < eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->notes; i++)
	{
		if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->type == eof_note_type)
		{
			if(eof_selection.track == eof_selected_track)
			{
				if(eof_selection.multi[i])
				{
					if((!v) || (v && eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->note))
					{
						last = i;
						count++;
					}
...
int eof_figure_part(void)
{
	...
	else if(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->notes <= 0)
	{
		return -1;
	}
...
//				eof_hover_key = (((mouse_x - eof_window_3d->x) / 11) / 7) * 12 + note[((mouse_x - eof_window_3d->x) / 11) % 7] + 36;
			...
			if(eof_hover_key >= 0)
			{
				if(mouse_b & 1)
				{
					if(KEY_EITHER_CTRL && (eof_selection.current < eof_song->vocal_track[0]->lyrics))
					{
						if(eof_count_selected_notes(NULL, 0) == 1)
						{
							eof_prepare_undo(EOF_UNDO_TYPE_LYRIC_NOTE);
							eof_song->vocal_track[0]->lyric[eof_selection.current]->note = eof_hover_key;
						}
					}
		...
		else
		{
			...
			{
				eof_hover_key = eof_song->vocal_track[0]->lyric[eof_selection.current]->note;
			}
		}
	}
	else
	{
		if(eof_hover_note >= 0)
		{
			eof_hover_key = eof_song->vocal_track[0]->lyric[eof_hover_note]->note;
		}
	}
}
...
//				rectfill(eof_window_note->screen, 0, 25 + 8, eof_window_editor->w - 1, eof_window_editor->h - 1, eof_color_gray);
				...
				for(i = 0; i < eof_song->vocal_track[0]->lyrics; i++)
				{
					if((eof_song->vocal_track[0]->lyric[i]->pos >= eof_song->catalog->entry[eof_selected_catalog_entry].start_pos) && (eof_song->vocal_track[0]->lyric[i]->pos <= eof_song->catalog->entry[eof_selected_catalog_entry].end_pos))
					{
						eof_lyric_draw_catalog(eof_song->vocal_track[0]->lyric[i], i == eof_hover_note_2 ? 2 : 0);
					}
				}
	...
	else
	{
		...
		if(eof_vocals_selected)
		{
			if(eof_selection.current < eof_song->vocal_track[0]->lyrics)
			{
				textprintf_ex(eof_window_note->screen, font, 2, ypos, eof_color_white, -1, "Lyric = %d : Pos = %lu : Length = %lu", eof_selection.current, eof_song->vocal_track[0]->lyric[eof_selection.current]->pos, eof_song->vocal_track[0]->lyric[eof_selection.current]->length);
				ypos += 12;
				textprintf_ex(eof_window_note->screen, font, 2, ypos, eof_color_white, -1, "Lyric Text = \"%s\" : Tone = %d (%s)", eof_song->vocal_track[0]->lyric[eof_selection.current]->text, eof_song->vocal_track[0]->lyric[eof_selection.current]->note, (eof_song->vocal_track[0]->lyric[eof_selection.current]->note != 0) ? eof_get_tone_name(eof_song->vocal_track[0]->lyric[eof_selection.current]->note) : "none");
			}
			else
			{
		...
		else
		{
			...
			{
				textprintf_ex(eof_window_note->screen, font, 2, 48, eof_color_white, -1, "Note = %d : Pos = %lu : Length = %lu", eof_selection.current, eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[eof_selection.current]->pos, eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[eof_selection.current]->length);
			}
			else
			{
...
void eof_render_3d_window(void)
{
	...
	{
		sz = -eof_music_pos / eof_zoom_3d + eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->solo[i].start_pos / eof_zoom_3d + eof_av_delay / eof_zoom_3d;
		sez = -eof_music_pos / eof_zoom_3d + eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->solo[i].end_pos / eof_zoom_3d + eof_av_delay / eof_zoom_3d;
		if((-100 <= sez) && (600 >= sz))
		{
			spz = sz < -100 ? -100 : sz;
...
//	int last_note = 0;
	...
	{	//Render 3D notes from last to first so that the earlier notes are in front
		if(eof_note_type == eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->type)
		{
			tr = eof_note_tail_draw_3d(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i], (eof_selection.multi[i] && eof_music_paused) ? 1 : i == eof_hover_note ? 2 : 0);
			eof_note_draw_3d(eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i], (eof_selection.track == eof_selected_track && eof_selection.multi[i] && eof_music_paused) ? 1 : i == eof_hover_note ? 2 : 0);
