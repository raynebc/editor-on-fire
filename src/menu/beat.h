...
int eof_menu_beat_push_offset_back_number_beats(unsigned long number);	//Calls eof_menu_beat_push_offset_back() enough times to add the specified number of beats, making a single undo state.  Returns nonzero on success
int eof_menu_beat_push_offset_back_several_beats(void);	//Prompts the user for a number of beats and calls eof_menu_beat_push_offset_back_number_beats() to push the offset back that many beats
int eof_menu_beat_push_offset_back_measure(void);	//Calls eof_menu_beat_push_offset_back_number_beats() to add one measure's worth of beats (assuming 4/4 if no time signature is in effect on the first beat), making a single undo state
...
int eof_menu_beat_push_offset_up(void);			//Calls eof_menu_beat_push_offset_up_logic() if the tempo map is not locked, creating an undo state
int eof_menu_beat_push_offset_up_number_beats(unsigned long number);	//Calls eof_menu_beat_push_offset_up_logic() enough times to remove the specified number of beats, making a single undo state.  Returns nonzero on success
int eof_menu_beat_push_offset_up_several_beats(void);	//Prompts the user for a number of beats and calls
int eof_menu_beat_push_offset_up_measure(void);	//Calls eof_menu_beat_push_offset_up_logic() enough times to remove one measure's worth of beats (assuming 4/4 if no time signature is in effect on the first beat), making a single undo state
...
void eof_add_or_edit_text_event(EOF_TEXT_EVENT *ptr, unsigned long flags, char *undo_made);
	...
	//	If (flags & EOF_EVENT_FLAG_RS_PHRASE) is true, the "Rocksmith phrase marker" option is automatically checked, otherwise that checkbox is initialized to clear
	//	Similarly, if (flags & EOF_EVENT_FLAG_RS_SECTION) is true, the "Rocksmith section marker" option is automatically checked
	//	Similarly, if (flags & EOF_EVENT_FLAG_RS_EVENT) is true, the "Rocksmith event marker" option is automatically checked
