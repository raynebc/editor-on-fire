...
int eof_menu_track_find_ch_sp_path(void)
{
	...
	int first_deploy_found = 0;
	unsigned long ctr, ctr2, index, tracksize, sp_count, numsolos;
	unsigned long validcount = 0, invalidcount = 0, pre_reject_count = 0;
	EOF_PHRASE_SECTION *sectionptr;
	int error = 0;
	char undo_made = 0;
	clock_t starttime = 0, endtime = 0;
	unsigned long sp_phrase_count;
	double sp_sustain;				//The number of beats of star power sustain, used to estimate whammy bonus star power when pre-screening solutions
	...
	unsigned long num_deployments;	//The number of star power deployments being tested in the current solution.  When all possible deployments of this number have been tested,
									// this counter will be incremented, up to a limit of max_deployments
	...
	if(!note_measure_positions || !note_beat_lengths || !deploybest || !deploytesting || !best || !testing || !sp_phrase_endings)
	...
	{	//For each note in the active track
		...
		{	//If the note is in the active difficulty
			...
			if((sp_count >= 2) && !first_deploy_found)
			{	//If the first star power deployment point wasn't found yet, but there are now at least two star power phrase's worth of star power
				first_deploy = index + 1;	//The next note (if any) will be the first one where star power can be deployed
				first_deploy_found = 1;
	...
	(void) snprintf(eof_log_string, sizeof(eof_log_string) - 1, "\tDetected maximum number of star power deployments is %lu", max_deployments);
	eof_log(eof_log_string, 2);
	...
	else
	{
		...
		(void) snprintf(eof_log_string, sizeof(eof_log_string) - 1, "\tEstimated maximum score without deploying star power is %lu.  First possible star power deployment is at note index %lu in this track difficulty.", worst_score, first_deploy);
		eof_log(eof_log_string, 1);
	}
	...
	{	//Continue testing until all solutions are tested, unless scoring the no deployment solution failed
		...
		{	//Starting with the first note at which star power deployment is possible
			testing->deploy[index]++;	//Invert star power deployment for this note compared to last examined solution
			if(testing->deploy[index] == 1)
			{	//If star power would be deployed at this note in the tested solution
				break;	//The solution has been incremented
			}
			testing->deploy[index] = 0;	//Otherwise star power was deployed at this note in the previously tested solution, ensure it's off and continue incrementing with the next note
		}
		if(index >= note_count)
		{	//If all solutions have been tested
			break;	//Stop testing them
		}
		...
		for(index = first_deploy; index < note_count; index++)
		{	//Starting with the first note at which star power deployment is possible
			if(testing->deploy[index])
			{	//If star power is to be deployed at this note
				num_deployments++;	//Count how many deployments are being tested
		...
		{	//If the solution was considered valid
			...
			{	//If this newly tested solution achieved a higher score than the current best, or if it matched the highest score but did so with fewer notes played during star power deployment
				...
				memcpy(best->deploy, testing->deploy, sizeof(unsigned char) * note_count);
			}
