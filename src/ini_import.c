...
int eof_import_ini(EOF_SONG * sp, char * fn, int function)
{
	...
	eof_log("\tChecking if INI file is UNICODE-8", 1);
	if (eof_is_unicode(textbuffer)) //If UNICODE-8 give error
		return 0;
...
int eof_is_unicode(const char *filename)
{
	...
	if (is_unicode)
		eof_log("\tfile is UNICODE-8", 1);
	else
		eof_log("\tfile is NOT UNICODE-8", 1);
