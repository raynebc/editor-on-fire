...
*/
	...
	{	//For each note in the track being evaluated
		...
		{
//			eof_log_casual("num_deployments is unexpectedly zero", 1, 1, 1);	//This condition would happen if there are no defined star power deployments, but scoring is still to be performed for the notes
		}
...
void eof_set_note_ch_sp_deploy_status(EOF_SONG *sp, unsigned long track, unsigned long note, int function, char *undo_made)
{
	...
	eflags = eof_get_note_eflags(sp, track, note);
	if(sp_deploy)
	...
	eof_set_note_eflags(sp, track, note, eflags);
}
