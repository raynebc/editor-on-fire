...
int eof_generate_hand_positions_current_track_difficulty(void)
{
	...
	(void) eof_pro_guitar_track_find_effective_fret_hand_position_definition(eof_song->pro_guitar_track[eof_song->track[eof_selected_track]->tracknum], eof_note_type, EOF_SEEK_POS, NULL, &diffindex, 0);
...
int eof_rs_export_common(EOF_SONG * sp, unsigned long track, PACKFILE *fp, unsigned short *user_warned, int target)
{
	...
	for(ctr = 0; ctr < sp->beats; ctr++)
	{	//For each beat in the chart
		unsigned long phraseid = ULONG_MAX;
