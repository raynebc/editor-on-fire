...
int eof_open_strum_enabled(unsigned long track);
	//Returns nonzero if the specified track is a legacy guitar/bass track or a BEATABLE track and has the open strum flag set
...
int eof_track_is_vocal_track(EOF_SONG *sp, unsigned long track);
	//Returns nonzero if the specified track is a vocal track
...
int eof_pro_guitar_note_derive_string_fingering(EOF_SONG *sp, unsigned long track, unsigned long note, unsigned char stringnum, unsigned char *result);
	...
	//Returns -1 if the note is supposed to be played open but has a defined fingering
	...
	//Returns -3 if the note conflicts with the active arpeggio/handshape by having a gem that isn't in the base chord
	//Returns -4 if the note conflicts with the active arpeggio/handshape by having a gem with a fret that differs from the base chord's fret for that string
	//Returns -5 if the note conflicts with the active arpeggio/handshape by having a gem with a fingering that differs from the base chord's fingering for that string
...
unsigned long eof_get_pos_num_notes_after_timestamp(EOF_SONG *sp, unsigned long track, unsigned long pos, unsigned long numnotes);
	...
	//By design of the moveR mechanism, if a note is at the same timestamp as the moveR phrase, that note is the first note in the scope of the mechanism
