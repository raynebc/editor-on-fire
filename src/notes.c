...
int eof_expand_notes_window_macro(char *macro, char *dest_buffer, unsigned long dest_buffer_size, EOF_TEXT_PANEL *panel)
{
	...
	if(eof_track_is_pro_guitar_track(eof_song, eof_selected_track))
		tp = eof_song->pro_guitar_track[tracknum];	//If the active track is a pro guitar track, cache the pointer to the track data
	else if(eof_vocals_selected)
		vp = eof_song->vocal_track[0];	//If the vocal track is active, cache the pointer to the track data
	...
	if(strcasestr_normal(macro, "IF_") == macro)
	{	//If this macro begins with "IF_" (case insensitive), it is a conditional macro
		return eof_expand_notes_window_conditional_macro(macro, dest_buffer, dest_buffer_size, panel);	//Let the conditional macro processing function handle it
	}
	...
	if(!ustricmp(macro, "CH_SP_MAX_DEPLOYMENT_COUNT_AND_RATIO"))
	{
		...
		if(eof_ch_sp_solution && eof_ch_sp_solution->score)
		{	//If the global star power solution structure is built and a score was determined
			count = eof_get_track_diff_size(eof_song, eof_selected_track, eof_note_type);	//Count the number of notes in the active track difficulty
...
{
	unsigned long tracknum, tracksize, ctr, ctr2, ctr3, notectr, lead = 0, rhythm = 0, bass = 0, stringnum, bitmask, diff, count = 0;
	...
	if((eof_log_level > 2) && !panel->logged)
	{	//If exhaustive logging is enabled and this panel hasn't been logged since it was loaded
		(void) snprintf(eof_log_string, sizeof(eof_log_string) - 1, "\t\t\tProcessing conditional Notes macro \"%s\"", macro);
		eof_log(eof_log_string, 3);
	}
	...
	{
		if((eof_song->track[eof_selected_track]->track_behavior == EOF_GUITAR_TRACK_BEHAVIOR) && (eof_song->track[eof_selected_track]->track_behavior != EOF_PRO_GUITAR_TRACK_BEHAVIOR) && !eof_track_is_ghl_mode(eof_song, eof_selected_track))
		{	//If the active track is a guitar track, but not a pro guitar track and not a GHL track
	...
	{
		if((eof_song->track[eof_selected_track]->track_behavior == EOF_GUITAR_TRACK_BEHAVIOR) && (eof_track_is_ghl_mode(eof_song, eof_selected_track) || tp))
		{	//If the active track is a guitar track and it's either a GHL track or a pro guitar track
			if(eof_track_is_ghl_mode(eof_song, eof_selected_track) || (eof_track_is_pro_guitar_track(eof_song, eof_selected_track)))
			{
				dest_buffer[0] = '\0';
				return 3;	//True
	...
	count_string = strcasestr_spec(macro, "IF_ACTIVE_TRACK_PG_NOTE_OCCURS_BEFORE_MILLIS_");	//Get a pointer to the text that would be the millisecond count
	if(count_string)
	{	//If the macro is this string
	...
	if(!ustricmp(macro, "IF_INACTIVE_TRACKS_HAVE_RS_WARNINGS"))
	{
		if(eof_notes_inactive_track_has_rs_warnings)
		{
			dest_buffer[0] = '\0';
			return 3;	//True
		}
	...
	if(!ustricmp(macro, "IF_INACTIVE_TRACKS_HAVE_RS_ERRORS"))
	{
		if(eof_notes_inactive_track_has_rs_errors)
		{
			dest_buffer[0] = '\0';
			return 3;	//True
		}
		...
		{	//For each track in the project
			...
			{	//For each tech note in the track
				...
				{	//If this tech note doesn't overlap a note on any string
					...
					dest_buffer[0] = '\0';
					retval = 3;	//True
					break;	//Stop processing the rest of this track so inactive tracks can be checked for warnings
				}
			}
		}
...
void eof_render_text_panel(EOF_TEXT_PANEL *panel, int opaque)
{
	...
	eof_notes_inactive_track_has_rs_warnings = eof_notes_inactive_track_has_rs_errors = 0;	//Reset these conditions
