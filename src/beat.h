...
int eof_get_beat(EOF_SONG * sp, unsigned long pos);	//Returns the beat number immediately before the specified position, or -1 if the timestamp does not occur within the chart
...
int eof_find_next_anchor(EOF_SONG * sp, int cbeat);	//Returns the beat number of the first first anchor that occurs after the specified beat, or -1 if there is no such anchor
