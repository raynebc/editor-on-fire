...
int eof_project_unsaved = 0;		//Set to true at the end of an import or project creation and reset to 0 whenever a save or load subsequently occurs
...
int eof_undo_apply(void)
{
	...
	eof_redo_count = 1;
	eof_change_count--;
	if(!eof_change_count && !eof_project_unsaved)
	{
		eof_changes = 0;
	}
	else
