...
const char *LYRICFORMATNAMES[NUMBEROFLYRICFORMATS+1]={"UNKNOWN LYRIC TYPE","SCRIPT","VL","RB MIDI","UltraStar","LRC","Vocal Rhythm","ELRC","KAR","Pitched Lyrics","Soft Karaoke","ID3 Lyrics","SRT Subtitle","XML","JamBand","Rocksmith XML","Rocksmith 2 XML","Plain script","QRC", "IMMERROCK LRC"};
...
//Continue reading lines until one begins with something other than #, then test for the text based formats (Script, UltraStar, LRC/ELRC)
	while(!feof(inf) && !readerrordetected)		//Until end of file is reached or fgets() returns an I/O error
	{
		...
		temp=strstr(buffer,"<QrcInfos>");	//Search for XML tag used in QRC lyrics
		if(temp && isxml)
		{	//If this tag AND the XML header tag were read
			free(buffer);
			fclose_err(inf);
			detectionlist->format=QRC_FORMAT;
			return detectionlist;
		}
