...
//					eof_mouse_drug = 1;
			...
			if((((eof_input_mode != EOF_INPUT_REX) && ((mouse_b & 2) || key[KEY_INSERT])) || (((eof_input_mode == EOF_INPUT_REX) && !KEY_EITHER_SHIFT && !KEY_EITHER_CTRL && (key[KEY_1] || key[KEY_2] || key[KEY_3] || key[KEY_4] || key[KEY_5])) && eof_rclick_released && (eof_pen_lyric.pos < eof_music_length))) || key[KEY_BACKSPACE])
			{
				...
				if(eof_hover_note >= 0)
				{
					...
					{
						eof_vocal_track_delete_lyric(eof_song->vocal_track[tracknum], eof_hover_note);
						eof_selection.current = EOF_MAX_NOTES - 1;
						eof_vocal_track_sort_lyrics(eof_song->vocal_track[tracknum]);
					}
	...
	for(i = 0; i < eof_song->vocal_track[tracknum]->lyrics; i++)
	{
		...
		{	//If the lyric would render at or after the left edge of the piano roll
			if(((eof_input_mode == EOF_INPUT_PIANO_ROLL) || (eof_input_mode == EOF_INPUT_REX)) && eof_music_paused)
			{
				if(eof_hover_note == i)
				{
					if(eof_lyric_draw(eof_song->vocal_track[tracknum]->lyric[i], ((eof_selection.track == EOF_TRACK_VOCALS) && eof_selection.multi[i] && eof_music_paused) ? 1 : i == eof_hover_note ? 2 : 3, eof_window_editor))
						break;	//Break if the function indicated that the lyric was rendered beyond the clip window
				}
				else
				{
					if(eof_lyric_draw(eof_song->vocal_track[tracknum]->lyric[i], ((eof_selection.track == EOF_TRACK_VOCALS) && eof_selection.multi[i] && eof_music_paused) ? 1 : i == eof_hover_note ? 2 : 0, eof_window_editor))
						break;	//Break if the function indicated that the lyric was rendered beyond the clip window
				}
			}
			else
