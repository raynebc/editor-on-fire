...
//determine the y axis location and graph height of each channel's graph
	...
	if(numgraphs == 1)
	{	//The y scaling logic needs different handling depending on how many channels are being displayed
...
//Decode OGG into memory
	...
	{
		eof_log("Waveform: Failed to parse OGG", 1);
		done = -1;
	}
	else
	{
		...
		else
		{
			*waveform = emptywaveform;					//Set all variables to value zero
			...
			else
			{
				...
				if(!waveform->numslices)
				{
					eof_log("Waveform: Error processing audio", 1);
					done = -1;
				}
				else
				{
