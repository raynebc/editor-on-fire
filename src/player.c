	...
	else
	{
		if(key[KEY_S] && (eof_count_selected_notes(NULL, 0) > 0))
		{
			...
			else
			{
				...
				{
					if(eof_selection.multi[i] && (eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->pos + eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->length > eof_music_end_pos))
					{
						eof_music_end_pos = eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->pos + eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->length;
					}
					if(eof_selection.multi[i] && (eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->pos < eof_music_rewind_pos))
					{
						eof_music_rewind_pos = eof_song->legacy_track[eof_song->track[eof_selected_track-1]->tracknum]->note[i]->pos;
					}
				}
			}
			eof_music_seek(eof_music_rewind_pos);
